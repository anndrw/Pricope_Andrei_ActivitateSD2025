...
void filtreazaHeap(Heap heap, int pozitieNod) {
	...
	int pozFiuStanga = 2 * pozitieNod + 1; // formula pentru a determina fiul din arbore din stanga
	int pozFiuDreapta = 2 * pozitieNod + 2; // formula pentru a determina fiul din arbore din dreapta

	int pozMax = pozitieNod; // presupunem si verificam cu if dupa | caz in care cautam maximul, la minim punem doar >
	if (pozFiuStanga < heap.nrMasini && heap.vector[pozMax].pret < heap.vector[pozFiuStanga].pret) {
		pozMax = pozFiuStanga;
	}
	if (pozFiuDreapta < heap.nrMasini && heap.vector[pozMax].pret < heap.vector[pozFiuDreapta].pret) { // pozFiuDreapta < heap.nrMasini punem in caz ca intram in masinile invizibile de exemplu nu exista pozitia in arbore
		pozMax = pozFiuDreapta;
	}

	if (pozMax != pozitieNod) { // interschimbare
